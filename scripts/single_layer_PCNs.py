...
for k in range(len(sample_sizes)):
    sample_size, batch_size, learning_iter, inference_iter = sample_sizes[k], batch_sizes[k], learning_iters[k], inference_iters[k]
    for ind, seed in enumerate(seeds):
        ...
        for i in range(5):
            ax[0, i].imshow(X[i].reshape((image_size, image_size)).cpu().detach().numpy(), cmap='gray', vmin=0, vmax=1)
            ax[0, i].axis('off')
            ax[1, i].imshow(X_c[i].reshape((image_size, image_size)).cpu().detach().numpy(), cmap='gray', vmin=0, vmax=1)
            ax[1, i].axis('off')
